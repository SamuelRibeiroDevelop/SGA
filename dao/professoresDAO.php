...
class professoresDAO
{
    ...
    public function remover($prof)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("DELETE FROM Professor WHERE idProfessor = :id");
            $statement->bindValue(":id", $prof->getIdProfessor());
    ...
    public function salvar($prof)
    {
        ...
        try {
            ...
            $statement->bindValue(":nome", $prof->getNomeProfessor());
            $statement->bindValue(":cargo", $prof->getCargoProfessor());
    ...
    public function atualizar($prof)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT idProfessor, Nome, Cargo FROM Professor WHERE idProfesor = :id");
            $statement->bindValue(":id", $prof->getIdProfessor());
            if ($statement->execute()) {
                $rs = $statement->fetch(PDO::FETCH_OBJ);
                $prof->setIdProfessor($rs->idProfessor);
                $prof->setNomeProfessor($rs->Nome);
                $prof->setCargoProfessor($rs->Cargo);
                return $prof;
            } else {
                throw new PDOException("Erro: Não foi possível executar a declaração sql");
            }
